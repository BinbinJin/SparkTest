...
//    println(precision+" "+recall+" "+correct)
  ...
  def rule(sc:SparkContext): Unit ={
    ...
    val score = sc.textFile("C:\\Users\\zjcxj\\Desktop\\2016ByteCup\\modelMerge\\MF_SVD_SVD++_FM_1_1_1_2.csv").map{x=>
      val info = x.split(",")
      val qid = info(0)
      val uid = info(1)
    ...
    score.leftOuterJoin(prst).map{x=>
      ...
      qid+","+uid+","+score
    }.repartition(1).saveAsTextFile("C:\\Users\\zjcxj\\Desktop\\2016ByteCup\\modelMerge\\MF_SVD_SVD++_FM_1_1_1_2_rule")
